...
class MassiveSource(MarketDataSource):
    """Real market data via Massive's REST snapshot endpoint, polled on a timer.

    The `massive` client is synchronous (urllib3-based), so every call is pushed off the event
    loop with asyncio.to_thread — otherwise it would stall the SSE stream and every other
    in-flight request for the duration of the HTTP call. See planning/MASSIVE_API.md.
    """
